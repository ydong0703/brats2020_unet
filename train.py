...
def train_loop(model,optimizer,scheduler,criterion,train_loader,device,epoch):
    ...
    for it,(images,masks) in enumerate(pbar):
        ...
        # # ✅ 添加 L1 正则项================================
        # l1_lambda = 1e-5  # 你可以尝试 1e-6 ~ 1e-3 之间的值
...
def main(args):
    ...
    optimizer = optim.SGD(model.parameters(),momentum=0.9, lr=0)
    ...
    if os.path.exists(args.weights):
        try:
            weight_dict = torch.load(args.weights, map_location=device, weights_only=False)
            model.load_state_dict(weight_dict['model'])
            optimizer.load_state_dict(weight_dict['optimizer'])
            print('Successfully loaded checkpoint:', args.weights)
        except Exception as e:
            print('⚠️ Failed to load checkpoint:', args.weights)
            print('ℹ️ Reason:', e)
            print('➡️ Training will start from scratch.')
...
if __name__ == '__main__':
    ...
    # ✅ 改为你的 .h5 数据路径
    parser.add_argument('--data_path', type=str, default='/content/drive/MyDrive/453GroupProject/brats-unet/processed_dataset')

    # ✅ 改为你生成的 train / valid / test txt
    parser.add_argument('--train_txt', type=str, default='/content/drive/MyDrive/453GroupProject/brats-unet/data_process/train.txt')
    parser.add_argument('--valid_txt', type=str, default='/content/drive/MyDrive/453GroupProject/brats-unet/data_process/valid.txt')
    parser.add_argument('--test_txt', type=str, default='/content/drive/MyDrive/453GroupProject/brats-unet/data_process/test.txt')
