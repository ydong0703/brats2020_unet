...
if __name__ == '__main__':
    ...
    test_save_path = '/content/drive/MyDrive/453GroupProject/brats-unet/predictions/unet/'
    save_mode_path = '/content/drive/MyDrive/453GroupProject/brats-unet/results/UNet.pth'

    os.makedirs(test_save_path, exist_ok=True)

    net = UNet(in_channels=4,num_classes=4).cuda()
    state_dict = torch.load(save_mode_path, map_location='cpu', weights_only=False)
    net.load_state_dict(state_dict['model'])

    print("init weight from {}".format(save_mode_path))
    net.eval()
    with open('/content/drive/MyDrive/453GroupProject/brats-unet/data_process/test.txt', 'r') as f:
        image_list = [
            os.path.join(data_path, x.strip() + '_mri_norm2.h5')  # ✅ 拼上 .h5 后缀
